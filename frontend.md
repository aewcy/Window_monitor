...
- **触发**: 顶部栏「网格视图」按钮 或 `⌘G`
- **叠加层**: 80vw × 75vh
- **内容**: 缩略图按日期分组显示 (auto-fill, minmax(180px, 1fr))
- **日期导航**: 顶栏日期芯片可点击跳转到对应日期位置
- **日期标签**: sticky 置顶显示日期 + 数量
- **卡片信息**: 左下角时间、右下角屏幕编号 (多屏时)
- **交互**:
  - hover 显示复选框 + 删除按钮
  - 全选 / 批量删除
  - 无限滚动 (滚到底部加载下一批 30 张)
...
- [x] 日历筛选后卡片内直接显示历史截图 (标题+上下切换)
- [x] 网格视图 overlay (80vw, 按日期分组+日期跳转+缩略图+全选+批量删除+无限滚动)
- [x] 配色系统 (6 强调色 × 5 背景色)
- [x] 底部统计栏
- [x] 系统日志查看
- [x] 确认弹窗 (删除操作)
