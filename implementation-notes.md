...
## 2026-07-17 网格截图程序与网址筛选

### 目标

- 在日历时段进入的网格视图中，按截图上报时附带的前台程序和浏览器网址筛选图片。
- 程序采用单层选择；浏览器采用“浏览器进程 - 主域名”的父子选择，并支持父项半选状态。
- 筛选必须先由 Server 执行、再分页，避免长时段内只加载前 200 张导致符合条件的图片遗漏。

### 实现方案

- 新增 `GET /api/screenshots/filter-options`，它根据当前 Agent、屏幕和时间范围汇总可用程序、浏览器和网址主域名及其截图数量。
- `GET /api/screenshots` 新增重复参数 `process`、`browser`、`domain`；三类已选条件取并集，并在 SQL 的 `LIMIT/OFFSET` 前生效。
- 域名匹配限制为完整主机名的 HTTP/HTTPS URL 边界，不以普通子串匹配，避免 `fakebilibili.com` 命中 `bilibili.com`。
- Dashboard 默认“全部”不发送筛选条件，因此包含旧截图和无前台标签截图；用户应用非全选时，按当前选项重新从 Server 完整加载结果。
- Agent 更新仍采用手动授权：发布新版本或部署 Server 不会自动创建更新任务，只有网页端点击“允许更新”后，Agent 才会领取并自动安装对应任务。

### 边缘情况与偏离说明

- 旧截图若没有程序或 URL 元数据，在默认全部视图中正常保留；一旦用户应用精确筛选，它们因没有可匹配标签而不会显示。
- 网址筛选按主域名展示，不拆分完整 URL；完整 URL 继续在图片右下角的信息面板中查看和复制。
- 当前筛选项按截图数据库中已有元数据统计，不修改日历显示的原始总截图数量。
