...
- 管理后台：`http://<服务器IP>:14325/`
- Agent 下载页：`http://<服务器IP>:14325/download`
- API 文档：`http://<服务器IP>:14325/docs`
- 健康检查：`http://<服务器IP>:14325/api/health`
...
```bash
curl http://127.0.0.1:14325/api/health
```

确认下载接口是否返回安装器：

```bash
curl -I http://127.0.0.1:14325/api/agent/download
```
