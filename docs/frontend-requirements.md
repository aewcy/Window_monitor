...
> 本节为后续开发需求，目标是在 Web 端控制 Agent 灰度更新，避免已部署被控机同时离线。
...
前端默认不提供“一键全部更新”作为主操作。

推荐交互：

1. 管理员选择一台 Agent。
2. 点击“允许更新”。
3. 二次确认，提示该 Agent 会短暂重启。
4. 前端轮询该 Agent 更新状态。
...
任意 Agent 更新失败时，批次状态显示为“已暂停”，避免继续更新其他机器。
...
短期推荐把更新入口放在 Agent 卡片或 Agent 详情区域，不单独新增复杂页面。

当版本、批次、历史记录变多后，再拆成独立“版本管理 / 更新管理”页面。
