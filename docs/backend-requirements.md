...
| VIEWER | 1s | Dashboard 有观察者，且本机已空闲超过 1 分钟 |
| LIGHT_IDLE | 10s | 无观察者，且本机空闲 1-5 分钟 |
| DEEP_IDLE | 60s | 无观察者，且本机空闲 5-30 分钟 |
| VERY_DEEP_IDLE | 600s | 无观察者，且本机空闲 30 分钟以上 |

服务端仍执行截图入库节流：同一 Agent、同一显示器在 2 秒窗口内只保留最早截图，避免 ACTIVE 高频采集造成存储暴涨。
